...
try { if(fin != null) { ht.load(fin); fin.close(); } }
...
name = br.readLine(); if(name.equals("quit")) continue;
	...
	ht.put(name, number); changed = true; } while(!name.equals("quit"));
	// If phone book data has changed, save it.
   if(changed) { FileOutputStream fout = new FileOutputStream("phonebook.dat");
