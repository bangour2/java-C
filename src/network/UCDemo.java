...
class UCDemo { 
	public static void main(String args[]) throws Exception { 
		int c;
		URL hp = new URL("http://cs.kennesaw.edu/~dgayler"); 
		URLConnection hpCon = hp.openConnection();

//header
		int j = 10; while(j>-1){
		System.out.println("Header "+hpCon.getHeaderField(j)); j--;}
...
//get content type 
		...
		// get content length 
		int len = hpCon.getContentLength(); if(len == -1) 
			System.out.println("Content length unavailable.");
		else System.out.println("Content-Length: " + len);
		if(len != 0) { System.out.println("=== Content ==="); 
		InputStream input = hpCon.getInputStream(); 
		int i = len; while (((c = input.read()) != -1)) { // && (--i > 0)) { 
			System.out.print((char) c); } input.close();
		}
